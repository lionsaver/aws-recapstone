...
Note: subnet group tab is in the left hand menu of AWS RDS page
...
Additional Configuration:
    - Initial Database Name  : database1  ***********IMPORTANT********
...
## this bucket plicy is not needed because ACLs is enabled.
...
"Principal": "*",
"Action": "s3:GetObject",
"Resource": "arn:aws:s3:::www.liondevops.click/*"
}
]
}

## not: wwww.liondevops.click bucket access control list ile public yapıldığından policy delete edildi.
...
git clone https://$TOKEN@github.com/lionsaver/capstone_project.git
...
apt-get install python3.7-dev default-libmysqlclient-dev -y
pip3 install -r requirements.txt
cd /home/ubuntu/aws-recapstone/src
python3 manage.py collectstatic --noinput
python3 manage.py makemigrations
python3 manage.py migrate
python3 manage.py runserver 0.0.0.0:80
